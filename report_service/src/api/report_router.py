...
@router.post(
    ...
    description=(
        ...
        "После вызова пользователю сразу возвращается сообщение "
        "о начале генерации отчета, в то время как сама генерация "
        "выполняется асинхронно в фоновом режиме. "
        "После того как отчет сгенерирован, "
        "он отправляется на почту пользователя, указанную в body."
    ),
...
async def generate_report(
    ...
    Returns:
        SuccessResponse: Сообщение, подтверждающее,
        что процесс генерации отчета был успешно запущен.
    """
    background_tasks.add_task(
        ReportRepository.create_report_and_send_to_email, email=body
    )
